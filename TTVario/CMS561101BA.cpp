...
CMS561101BA::CMS561101BA() 
{
}

void CMS561101BA::init(uint8_t address) 
{  
  _addr =  address;
...
float CMS561101BA::getPressure(uint8_t OSR) 
{
  // see datasheet page 7 for formulas
  uint32_t rawPress = rawPressure(OSR);
  if(rawPress == NULL)  return NULL;
...
float CMS561101BA::getTemperature(uint8_t OSR) 
{
  // see datasheet page 7 for formulas
  int64_t dT = getDeltaTemp(OSR);
  
  if(dT != NULL) return (2000 + ((dT * _C[5]) >> 23)) / 100.0;
  else return NULL;
}

int32_t CMS561101BA::getDeltaTemp(uint8_t OSR)
{
  uint32_t rawTemp = rawTemperature(OSR);
  if(rawTemp != NULL) return (int32_t)(rawTemp - ((uint32_t)_C[4] << 8));
  else return NULL;
}
...
uint32_t CMS561101BA::rawPressure(uint8_t OSR) 
{
...
uint32_t CMS561101BA::rawTemperature(uint8_t OSR) 
{
  unsigned long now = micros();
...
void CMS561101BA::startConversion(uint8_t command)
{
  i2c_writeReg(_addr,command,0);
}

uint32_t CMS561101BA::getConversion(uint8_t command) 
{

  union {uint32_t val; uint8_t raw[4]; } conversion = {0};

  i2c_read_reg_to_buf(_addr,0,conversion.raw,(uint8_t)MS561101BA_D1D2_SIZE);

  conversion.raw[3] = conversion.raw[0];
  conversion.raw[0] = conversion.raw[2];
  conversion.raw[2] = conversion.raw[3];
  conversion.raw[3] = 0;
...
*/
int CMS561101BA::readPROM() 
{
  uint8_t d[2];
  for (int i=0;i<MS561101BA_PROM_REG_COUNT;i++) 
  {
    
    i2c_read_reg_to_buf(_addr,MS561101BA_PROM_BASE_ADDR + (i * MS561101BA_PROM_REG_SIZE),
                        d,(uint8_t) MS561101BA_PROM_REG_SIZE);

    _C[i] = ((int)d[0] << 8) | (int)d[1];
  }
...
*/
void CMS561101BA::reset() 
{
  i2c_writeReg(_addr,MS561101BA_RESET,0);
}
